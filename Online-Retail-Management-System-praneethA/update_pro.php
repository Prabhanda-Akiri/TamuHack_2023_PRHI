...
<html lang="en">
  <head>
    ...
    <title>Admin Home Page| Update Product Status</title>
      ...
      <div class="row">
        <nav class="col-md-2 d-none d-md-block bg-light sidebar">
          <div class="sidebar-sticky">
            <ul class="nav flex-column">
              ...
              <li class="nav-item">
                <a class="nav-link active" href="update_pro.php">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Update Product Status</h1>
          ...
          <div class="table-responsive">
            <table class="table table-striped table-sm">
              ...
              <tbody>
                <?php
                    $cek = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM product where product_status='pending'"));
                    $data = mysqli_query(
                    $connect,"SELECT * FROM product where product_status='pending'");
                if ($cek>0)
                {     
                  while($row = mysqli_fetch_assoc($data)){
                    $id=$row['supplier_id'];
                    $supplier=mysqli_fetch_array(mysqli_query($connect,"SELECT s_name FROM supplier WHERE s_id=$id"));       
                    echo '<tr>
                    ...
                    echo '<td>'.$supplier['s_name'].'</td>
                      ...
                      <td><a href="acc_rej.php?acc=1&prod='.$row['product_id'].'">Accept</a></td>
                      <td><a href="acc_rej.php?acc=0&prod='.$row['product_id'].'">Reject</a></td>
                    </tr>';
                     }
                   }
                else
                die('No pending products to display. ' . mysqli_error($connect));
                  ?>
              </tbody>
            </table>
          </div>
